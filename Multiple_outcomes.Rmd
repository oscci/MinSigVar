...
Correlated variables were simulated using in the R programming language [@rcoreteam2020a]. The script to generate and analyse simulated data is available on https://github.com/oscci/MinSigVar, and more technical details are provided in the Appendix. Two approaches to modeling correlated variables were compared, Method M and Method L.  
...
method='L'
fignum<-fignum+1
plot(powertab$ES,powertab$obsES,col=as.factor(powertab$corr),xlab='True ES',ylab='Observed ES',main=paste0('Figure ',fignum,': Observed effect sizes with Method ',method))
abline(a=0,b=1,lty=2)
legend(.05,.6, legend=c('0','.2','.4','.6','.8'),title='correlation',
       col=1:5, lty=1, cex=0.8)
if(method=='L'){
text(.1,.2,'For Method L, True ES \nis effect size for latent factor',cex=.8)
}

```

For Method L, the observed effect size for the outcome measures will be lower than the effect size specified for the latent variable, because they are imperfect indicators. As shown in Figure `r fignum`, in the limiting case where the correlation is specified as zero, the observed effect size for observed outcomes will be zero, regardless of the effect size of the latent variable, because they do not share any variance with the latent variable. As the correlation between outcomes increases, the observed effect sizes approaches the true effect size in the latent variable. This made it potentially problematic when comparing power of a single outcome measure with a suite of measures, as this requires us to specify an effect size. The average observed effect size for all measures in a given condition was computed, and used as the basis for comparisons of efficiency between single and multiple measure scenarios. 

### Data reduction    
For both Method M and L, principal components were computed for all outcome variables, using the base R function _prcomp_ from the _stats_ package. The size of the suite of outcome variables entered into the analysis ranged from 2 to 12 in step of 2. Thus PC2 is a principal component based on the first two outcome measures, PC4 based on the first four outcome measures, and so on.  
...
mycap <- paste0('Table ',tabnum,': Values of MinNSig from Method M')
ft <- set_caption(ft,mycap)
ft
ft2<-flextable(shorttabL)
mycap <- paste0('Table ',tabnum,': Values of MinNSig from Method L')
...
par(mfrow=c(3,3))
  ...
  for (nvar in 1:length(alloutcomes)){
    xrange <- (e-1)*length(fpcols)+fpcols
    yrange <-fpcols
    xcol<-firstcol+nvar-1
    
    plot(shortpower[xrange,xcol],shortpower[yrange,xcol],main=paste0('Method ',method,' ES:',myES[e],' Nvar:',alloutcomes[nvar]),col=shortpower$colorcol,pch=shortpower$shapecol,xlim=c(0,1),ylim=c(.001,.1),log='y',xlab='Power',ylab='FP rate (log scale)')
    if(nvar>1){
    legend("topleft", legend=c('0','.2','.4','.6','.8'),pch  =1,
           col=mycols,  cex=0.75,title="Correlation",horiz=TRUE) #text for labels to save space
    }
    legend("bottomleft", legend=myN,pch=myshapes,
           col=1,  cex=0.75,title="Sample size",horiz=TRUE)
...
```{r doplots,echo=F,warning=F,fig.width=7,fig.asp=1,}
plotpower('M')

```

`r fignum <- fignum+1`
An efficient method is one that gives power of .8 or above, and a false positive rate of .05 or less, i.e. the results should cluster in the bottom right quadrant. Power is dependent on sample size, as is evident from these plots where the unfilled figures, corresponding to the smallest sample size of 20 per group,fail to achieve power above .8, except at the highest effect size of .7.  It is evident from inspection that when method M is used, the Adjust NVar approach compares favourably with the single outcome (Nvar 1) case, tending to give higher power and lower false positive rates, especially when there are between 4 and 6 variables with a moderate degree of intercorrelation.  
...
```
`As noted above, method L is more realistic, but because the effect size is computed from a latent variable, with outcome measures as indirect indicators, the observed effect sizes for individual variables in a suite will vary, depending on the correlation between the variables. For the case where there is a single primary outcome, the average effect size in multiple outcome simulations when correlation = .8 was used to compute power, using the _pwr.t.test_ function from the _pwr_ package in R.

 Method L provides power estimates that are more in keeping with the single outcome case, but achieves this level of power while providing a more stringent control of false positives. 
