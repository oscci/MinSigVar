...
method <- "L1"
alpha <- .05
nsim <- 100 #number of runs for each combination of mycorr, mynvar,myES and myN.
    ...
    for (rval in 1:length(mycorr)){
      for (e in myES){
        for(n in myN){
          thisrow <- thisrow+1
          c<-mycorr[rval]
          u <-usecorr[rval] #this is a hangover from when we tried to include c=0; c and u are the same here
          p.df$run[thisrow] <- s
          p.df$corr[thisrow] <- c
          p.df$ES[thisrow]<-e
          p.df$Nsub[thisrow]<-n
...
if (method=='L1'){
            ...
            I <- makecorL(n,outcomes,sqrt(c),e/sqrt(u),1)  #group I is intervention group: has ES added to mylatent value. Note that to get correct E for the observed vars, need to use e/sqrt(u)
...
method <-1
alpha<-.05
nupname<-paste0('data/simulated_pMEff_L',method,'.csv')
pfile<-here(nupname)
p.df<-read.csv(pfile)
...
powertab$meancorr<-NA  #mean correlation off diagonal - for method 1 should be close to corr
...
if(method==3){power3<- .5*(power3+.05)}
if(method==3){power5<- .5*(power5+.05)}
if(method==3){power8<- .5*(power8+.05)}

  plotbit <- filter(powertab,myN==n)
  #make long form with the 3 methods stacked above each other
...
p<-p + facet_grid( ~ mycorr)



plotname<-here(paste0('Figures/Method_',method,'_power_N_',n,'.jpg'))
